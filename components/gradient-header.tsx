...
export default function GradientHeader({ title, subTitle, children }: GradientHeaderProps) {
    return (
        <div className="relative overflow-hidden rounded-2xl bg-linear-to-r from-blue-500 via-purple-500 to-pink-500 p-5 sm:p-8 text-white mb-8">
            <div className="relative z-10">
                <h1 className="text-2xl sm:text-4xl font-bold">{title}</h1>
                {subTitle && (
                    <p className="mt-2 text-sm sm:text-lg text-blue-100 max-w-2xl">{subTitle}</p>
                )}
                {children}
            </div>
